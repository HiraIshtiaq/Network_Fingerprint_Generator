...
def generate_fingerprint(url, features):
    try:
        fingerprint = {
            ...
            'dns_queries': list(set(features.get('dns_queries', []))),
            ...
            'packet_sizes_sample': features.get('packet_sizes', [])[:100],
            'inter_arrival_times': features.get('inter_arrival_times', [])[:100],
            'behavior_label': 'Pending',
            'confidence': 0.0,
            'classification_characteristics': []
        }
        
        # Normalize protocol percentages
        ...
        if proto_dist:
            ...
            if abs(total - 100) > 0.1 and total > 0:
                for proto in proto_dist:
                    proto_dist[proto] = round((proto_dist[proto] / total) * 100, 2)
        
        # Generate hash
        hash_data = {k: v for k, v in fingerprint.items() 
                     if k not in ['behavior_label', 'confidence', 'classification_characteristics']}
